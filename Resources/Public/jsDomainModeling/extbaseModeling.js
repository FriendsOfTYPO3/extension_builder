var extbaseModeling_wiringEditorLanguage = {
  ...
  smdUrl: "/typo3/ajax/extensionBuilder/wireEditor",
  layerOptions: {},
  modules: [],
};
...
YAHOO.util.Event.onAvailable("extensionDependencies-field", function () {
  ...
  YAHOO.util.Event.addListener(
    "targetVersionSelector-field",
    "change",
    function (event) {
      var dependenciesField = document.querySelector(
        "#extensionDependencies-field",
      );

      var dependencies = dependenciesField.value
        .split("\n")
        .map(function (dependency) {
          var parts = dependency.split("=>");
          return parts[0].indexOf("typo3") > -1
            ? "typo3 => " + event.target.value
            : dependency;
        });
  ...
  YAHOO.util.Event.addListener("toggleAdvancedOptions", "click", function () {
    if (!advancedMode) {
      ...
      document.querySelector(
        "#toggleAdvancedOptions .simpleMode",
      ).style.display = "none";
      document.querySelector(
