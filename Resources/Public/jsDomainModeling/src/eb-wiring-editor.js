...
export class EbWiringEditor extends LitElement {
            ...
            for (const file of preview.modifiedFiles) {
                ...
                for (const c of file.changes ?? []) {
                    if (c.type === 'renamed') {
                        ...
                        lines.push('      − ' + c.method + ' (removed)\n');
                    } else if (c.type === 'added') {
                        lines.push('      + ' + c.method + ' (added)\n');
                    }
                }
            }
        }
    ...
    async save(extraParams = {}) {
                ...
                Modal.confirm(
                    ...
                    [
                        { text: 'Cancel', btnClass: 'btn-default', trigger: () => Modal.dismiss() },
                        {
                            ...
                            trigger: () => {
                                Modal.dismiss();
                                this.save({ ...extraParams, _previewDone: true });
                            },
                        },
                    ]
                );
        ...
        if (data.confirm) {
            Modal.confirm('Warning', data.confirm, Severity.warning, [
                { text: 'Cancel', btnClass: 'btn-default', trigger: () => Modal.dismiss() },
                {
                    text: 'Save anyway',
                    btnClass: 'btn-warning',
    ...
    addModelObject() {
        const layer = this.shadowRoot.querySelector('eb-layer');
        if (layer) {
            layer.addContainer(modelObjectModule.container);
        }
    }
