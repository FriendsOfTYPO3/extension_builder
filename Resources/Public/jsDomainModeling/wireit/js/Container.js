  ...
  WireIt.Container.prototype = {
    ...
    setOptions: function (options) {
      ...
      this.options.draggable =
        typeof options.draggable == "undefined" ? true : options.draggable;
      this.options.position = options.position || [100, 100];
      this.options.className = options.className || CSS_PREFIX + "Container";

      this.options.ddHandle =
        typeof options.ddHandle == "undefined" ? true : options.ddHandle;
      this.options.ddHandleClassName =
        options.ddHandleClassName || CSS_PREFIX + "Container-ddhandle";

      this.options.resizable =
        typeof options.resizable == "undefined" ? true : options.resizable;
      this.options.resizeHandleClassName =
        options.resizeHandleClassName || CSS_PREFIX + "Container-resizehandle";

      this.options.width = options.width; // no default
      this.options.height = options.height;

      this.options.close =
        typeof options.close == "undefined" ? true : options.close;
      this.options.closeButtonClassName =
        options.closeButtonClassName ||
        CSS_PREFIX +
          "Container-closebutton t3js-icon icon icon-size-small icon-state-default icon-actions-edit-delete";

      this.options.title = options.title; // no default

      this.options.icon = options.icon;

      this.options.preventSelfWiring =
        typeof options.preventSelfWiring == "undefined"
          ? true
    ...
    render: function () {
      ...
      if (this.options.ddHandle) {
        ...
        if (this.options.title) {
          this.ddHandle.appendChild(
            WireIt.cn("span", null, null, this.options.title),
          );
        }
      ...
      if (this.options.resizable) {
        ...
        this.ddResizeHandle = WireIt.cn("div", {
          className: this.options.resizeHandleClassName,
        });
