(function () {
  ...
  YAHOO.extend(WireIt.ModuleProxy, YAHOO.util.DDProxy, {
    ...
    onDragDrop: function (e, ddTargets) {
      ...
      this._WiringEditor.addModule(this._module, [
        pos[0] - layerPos[0],
        pos[1] - layerPos[1],
      ]);
    },
  ...
  WireIt.WiringEditor = function (options) {
    ...
    this.alertPanel.setBody(
      "<div id='wireEditorMessageBox'></div><button id='alertPanelButton'>Ok</button>",
    );
    ...
    this.confirmPanel.setBody(
      "<div id='wireEditorConfirmMessageBox'></div><button id='confirmPanelButton'>Ok</button>&nbsp;&nbsp;<button id='confirmPanelCancelButton'>Cancel</button>",
    );
    ...
    this.confirmUpdatesPanel.setBody(
      "<div id='wireEditorConfirmUpdatesMessageBox'></div><button id='confirmUpdatePanelButton'>Ok</button>&nbsp;&nbsp;<button id='confirmUpdatePanelCancelButton'>Cancel</button>",
    );
    ...
    this.showSpinnerPanel.setBody(
      '<img src="/_assets/705f31da96f0870eac9db412ee92df1f/jsDomainModeling/wireit/images/loading.gif" />',
    );
  ...
  WireIt.WiringEditor.prototype = {
    ...
    setOptions: function (options) {
      ...
      this.options.layerOptions.parentEl = layerOptions.parentEl
        ? layerOptions.parentEl
        : Dom.get("modelingLayer");
    ...
    loginCheck: function (o) {
      ...
      if (result.login.timed_out || result.login.will_time_out) {
        if (typeof parent.TYPO3.loginRefresh != "undefined") {
          ...
          o.argument.editor.alert(
            "Login expired",
            "Your login is expired. Please refresh your login in a separate browser window and save again",
          );
        }
      } else {
    ...
    validateModels: function (value) {
      ...
      if (value.working.modules) {
        for (
          var modelIndex = 0;
          modelIndex < value.working.modules.length;
          modelIndex++
        ) {
          ...
          if (!modelName || modelName.length < 2) {
            this.alert(
              this.localize("modeler_invalidConfigurationTitle"),
              this.localize("modeler_missingModelName"),
            );
            return false;
          }
          if (modelNames[modelName] !== undefined) {
            this.alert(
              this.localize("modeler_duplicateModelNamesTitle"),
              '2 x "' + modelName + '"',
            );
            return false;
          }
          ...
          if (!modelForm.validate()) {
            this.alert(
              this.localize("modeler_invalidConfigurationTitle"),
              this.localize("modeler_invalidModelConfiguration") + modelName,
            );
            return false;
          }
          propertyNames = {};
          for (
            var propertyIndex = 0;
            propertyIndex < model.propertyGroup.properties.length;
            propertyIndex++
          ) {
            propertyName =
              model.propertyGroup.properties[propertyIndex].propertyName;
            if (propertyNames[propertyName] !== undefined) {
              this.alert(
                this.localize("modeler_duplicatePropertyNamesTitle"),
                '2 x "' + propertyName + '" in model "' + modelName + '"',
              );
              return false;
            }
          ...
          for (
            var relationIndex = 0;
            relationIndex < model.relationGroup.relations.length;
            relationIndex++
          ) {
            propertyName =
              model.relationGroup.relations[relationIndex].relationName;
            if (propertyNames[propertyName] !== undefined) {
              this.alert(
                this.localize("modeler_duplicatePropertyNamesTitle"),
                '2 x "' + propertyName + '" in model "' + modelName + '"',
    ...
    confirmUpdates: function (title, message) {
      ...
      if (!this.updateEventListenerAdded) {
        ...
        Event.addListener(
          "confirmUpdatePanelCancelButton",
          "click",
          function () {
            this.confirmUpdatesPanel.hide();
          },
    ...
    onDelete: function () {
      if (confirm("Are you sure you want to delete this wiring ?")) {
        ...
        this.service.deleteWiring(
          { name: value.name, language: this.options.languageName },
          {
      ...
      if (lang.isArray(pipe.modules)) {
        ...
        for (i = 0; i < pipe.modules.length; i++) {
          ...
          if (this.modulesByName[m.name]) {
            ...
            throw new Error(
              "WiringEditor: module '" + m.name + "' not found !",
            );
          }
        }
      ...
      for (i = 0; i < this.layer.wires.length; i++) {
        ...
        var wireObj = {
          ...
          tgt: {
            ...
            uid: this.getUidForTerminal(wire.terminal2),
          },
        };
    ...
    getUidForTerminal: function (terminal) {
      if (terminal.el.getAttribute("title") === "SOURCES") {
        // id of the module
        return Dom.get(terminal.el)
          .parents(".WireIt-Container")[0]
          .querySelector('input[name="uid"]').value;
      }
      // id of the wrapper of the first field in the fieldset
      return Dom.get(terminal.el)
        .parents(".relationGroup")[0]
        .querySelector('input[name="uid"]').value;
    },
  };
})();
