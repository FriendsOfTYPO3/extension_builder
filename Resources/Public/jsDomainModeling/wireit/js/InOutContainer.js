...
YAHOO.lang.extend(WireIt.InOutContainer, WireIt.Container, {
  ...
  setOptions: function (options) {
    ...
    this.options.className =
      options.className || "WireIt-Container WireIt-InOutContainer";

    // Overwrite default value for options:
    this.options.resizable =
      typeof options.resizable == "undefined" ? false : options.resizable;
  ...
  render: function () {
    ...
    for (var i = 0; i < this.options.inputs.length; i++) {
      ...
      this.bodyEl.appendChild(
        WireIt.cn("div", null, { lineHeight: "30px" }, input),
      );
    }
    ...
    for (i = 0; i < this.options.outputs.length; i++) {
      ...
      this.options.terminals.push({
        ...
        offsetPosition: {
          right: -14,
          top: 3 + 30 * (i + 1 + this.options.inputs.length),
        },
        ddConfig: {
          type: "output",
          allowedTypes: ["input"],
      ...
      this.bodyEl.appendChild(
        WireIt.cn(
          "div",
          null,
          { lineHeight: "30px", textAlign: "right" },
          output,
        ),
      );
    }
