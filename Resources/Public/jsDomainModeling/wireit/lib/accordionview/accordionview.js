(function () {
  ...
  YAHOO.extend(AccordionView, YAHOO.util.Element, {
    ...
    initList: function (el, oAttr) {
      ...
      var aCollectedItems = [];
      var aListItems = el.getElementsByTagName(
      ...
      for (var i = 0; i < aListItems.length; i++) {
        if (YUD.hasClass(aListItems[i], "nopanel")) {
          aCollectedItems.push({
            label: "SINGLE_LINK",
            content: aListItems[i].innerHTML
              .replace(/^\s\s*/, "")
              .replace(/\s\s*$/, ""),
          });
        } else {
          if (aListItems[i].parentNode === el) {
            for (
              var eHeader = aListItems[i].firstChild;
              eHeader && eHeader.nodeType != 1;
              eHeader = eHeader.nextSibling
            ) {
              // This loop looks for the first non-textNode element
            }
            if (eHeader) {
              for (
                var eContent = eHeader.nextSibling;
                eContent && eContent.nodeType != 1;
                eContent = eContent.nextSibling
              ) {
                // here we go for the second non-textNode element, if there was a first one
              }
              aCollectedItems.push({
                label: eHeader.innerHTML,
    ...
    initEvents: function () {
      ...
      if (this._ARIACapable) {
        this.on("keypress", function (ev) {
          var eCurrentPanel = YUD.getAncestorByClassName(
            YUE.getTarget(ev),
            this.CLASSES.PANEL,
          );
    ...
    addPanel: function (oAttr, nIndex) {
      var oPanelParent = document.createElement(
        this.CONFIG.ITEM_WRAPPER_TAG_NAME,
      );
      ...
      if (oAttr.label === "SINGLE_LINK") {
        ...
        elPanelLink.id =
          this.get("element").id + "-" + this._idCounter + "-label";
        ...
        var elPanelContent = document.createElement(
      ...
      if (this.get("collapsible") === false) {
        if (this.get("expandable") === true) {
          ...
          for (var i = 0; i < aItems.length; i++) {
            if (
              YUD.hasClass(aItems[i].firstChild, this.CLASSES.ACTIVE) &&
              i !== nIndex
            ) {
    ...
    _onClick: function (arg) {
      ...
      if (arg.nodeType === undefined) {
        ...
        if (
          !YUD.hasClass(ev, this.CLASSES.TOGGLE) &&
          !YUD.hasClass(ev, this.CLASSES.INDICATOR)
        ) {
          return false;
        }
      ...
      function iehide(el, sHide) {
        if (that._ie) {
          var aInnerAccordions = YUD.getElementsByClassName(
            that.CLASSES.ACCORDION,
            that.CONFIG.TAG_NAME,
            el,
          );
      ...
      function toggleItem(el, elClicked) {
        ...
        if (this.get("animate")) {
          ...
          var nSpeed = this.get("animationSpeed")
            ? this.get("animationSpeed")
            : 0.5;
          var sEffect = this.get("effect")
            ? this.get("effect")
      ...
      var eTargetListNode =
        elClickedNode.nodeName.toUpperCase() === "SPAN"
          ? elClickedNode.parentNode.parentNode
          : elClickedNode.parentNode;

      var containedPanel = YUD.getElementsByClassName(
      ...
      if (this.get("expandable") !== true) {
        var nLength = this._panels.length;
        for (var i = 0; i < nLength; i++) {
          var bMustToggle = YUD.hasClass(
            this._panels[i].firstChild.nextSibling,
            this.CLASSES.HIDDEN,
          );
          if (!bMustToggle) {
