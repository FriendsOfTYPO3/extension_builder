...
(function () {
  var isOpera9 = window.opera && navigator.userAgent.match(/Opera\/9/), // It seems to be faster when the hacked methods are used. But there are artifacts with Opera 10.
    ...
    proto = window.CanvasRenderingContext2D
  ...
  function getElementStyle(e) {
    ...
    else if (e.currentStyle) e.computedStyle = e.currentStyle;
  ...
  ctxt.getFace = function (family, weight, style) {
    if (
      ...
      this.faces[family][weight][style]
    )
    ...
    if (xhr.status == 200) {
      ...
      throw (
        "Unable to load the font [" + family + " " + weight + " " + style + "]"
      );
    return false;
  };
  ...
  proto.getTextExtents = function (text, style) {
    ...
    for (i = 0; i < text.length; i++) {
      glyph =
        face.glyphs[text.charAt(i)] ||
        face.glyphs[ctxt.options.fallbackCharacter];
