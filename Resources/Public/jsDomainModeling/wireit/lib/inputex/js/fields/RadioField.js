     ...
     */
    {
      ...
      setOptions: function (options) {
        ...
        if (lang.isUndefined(options.allowAny) || options.allowAny === false) {
          ...
          this.options.allowAny.validator = lang.isFunction(
          ...
          this.options.allowAny.value = !lang.isUndefined(
            options.allowAny.value,
          )
            ? options.allowAny.value
            : "";
        }
        ...
        this.options.values = lang.isArray(options.values)
          ? options.values
          : options.choices;
      },
      ...
      renderComponent: function () {
        ...
        for (var i = 0; i < this.options.choices.length; i++) {
          ...
          var radioId = this.divEl.id
            ? this.divEl.id + "-field-opt" + i
            : YAHOO.util.Dom.generateId();

          var radio = inputEx.cn("input", {
            id: radioId,
            type: "radio",
          ...
          var label = inputEx.cn(
            "label",
            {
              for: radioId,
              className: "inputEx-RadioField-rightLabel",
            },
            null,
        ...
        if (this.options.allowAny) {
          ...
          if (YAHOO.env.ua.ie) {
            this.radioAny = document.createElement(
              "<input type='radio' name='" + this.options.name + "'>",
            );
          } else {
            this.radioAny = inputEx.cn("input", {
              type: "radio",
              name: this.options.name,
            });
          }
          ...
          this.anyField = new inputEx.StringField({
            value: this.options.allowAny.value,
          });
          ...
          if (this.options.allowAny.separators) {
            var sep = inputEx.cn(
              ...
              null,
              { margin: "3px" },
              this.options.allowAny.separators[0] || "",
            );
          ...
          if (this.options.allowAny.separators) {
            var sep = inputEx.cn(
              ...
              null,
              { margin: "3px" },
              this.options.allowAny.separators[1] || "",
            );
            Dom.setStyle(sep, "float", "left");
            div.appendChild(sep);
          }
      ...
      initEvents: function () {
        ...
        if (this.anyField) {
          this.anyField.updatedEvt.subscribe(
            function (e) {
      ...
      setValue: function (value, sendUpdatedEvt) {
        ...
        inputEx.StringField.superclass.setValue.call(
          this,
          value,
          sendUpdatedEvt,
        );
