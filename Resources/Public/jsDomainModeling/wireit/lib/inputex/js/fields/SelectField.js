  ...
  lang.extend(
     ...
     */
    {
      ...
      setOptions: function (options) {
        ...
        this.options.multiple = lang.isUndefined(options.multiple)
          ? false
          : options.multiple;
        ...
        for (var i = 0, length = options.selectValues.length; i < length; i++) {
          ...
          this.options.selectOptions.push(
            "" +
              (options.selectOptions &&
              !lang.isUndefined(options.selectOptions[i])
                ? options.selectOptions[i]
                : options.selectValues[i]),
          );
        }
      },
      ...
      setValue: function (value, sendUpdatedEvt) {
        ...
        inputEx.SelectField.superclass.setValue.call(
          this,
          value,
          sendUpdatedEvt,
        );
      ...
      addOption: function (config) {
        ...
        var option =
          "" +
          (!lang.isUndefined(config.option) ? config.option : config.value);
        ...
        this.options.selectValues = this.options.selectValues
          .slice(0, position)
          .concat([value])
          .concat(this.options.selectValues.slice(position, nbOptions));
        this.options.selectOptions = this.options.selectOptions
          .slice(0, position)
          .concat([option])
          .concat(this.options.selectOptions.slice(position, nbOptions));
      ...
      removeOption: function (config) {
        ...
        if (!lang.isNumber(position)) {
          throw new Error(
            "SelectField : invalid or missing position, option or value in removeOption",
          );
        }
