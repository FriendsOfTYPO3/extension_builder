(function () {
  ...
  lang.extend(
     ...
     */
    {
      ...
      setOptions: function (options) {
        ...
        this.options.strengthIndicator = YAHOO.lang.isUndefined(
          options.strengthIndicator,
        )
          ? false
          : options.strengthIndicator;

        // capsLockWarning
        this.options.capsLockWarning = YAHOO.lang.isUndefined(
          options.capsLockWarning,
        )
          ? false
          : options.capsLockWarning;
      },
      ...
      renderComponent: function () {
        ...
        if (this.options.capsLockWarning) {
          this.capsLockWarning = inputEx.cn(
            "div",
            { className: "capsLockWarning" },
            { display: "none" },
            inputEx.messages.capslockWarning,
          );
        ...
        if (this.options.strengthIndicator) {
          this.strengthEl = inputEx.cn(
            "div",
            { className: "inputEx-Password-StrengthIndicator" },
            null,
            inputEx.messages.passwordStrength,
          );
          this.strengthBlocks = [];
          for (var i = 0; i < 4; i++) {
            this.strengthBlocks[i] = inputEx.cn("div", {
              className: "inputEx-Password-StrengthIndicatorBlock",
            });
      ...
      setConfirmationField: function (passwordField) {
        ...
        this.options.messages.invalid =
          inputEx.messages.invalidPasswordConfirmation;
        this.options.confirmPasswordField.options.confirmationPasswordField =
          this;
      },
      ...
      getStateString: function (state) {
        if (
          ...
          return (
            inputEx.messages.invalidPassword[0] +
            this.options.minLength +
            inputEx.messages.invalidPassword[1]
          );
        }
      ...
      onKeyPress: function (e) {
        ...
        if (this.options.capsLockWarning) {
          ...
          var displayWarning =
            (which >= 65 && which <= 90 && !shift_status) ||
      ...
      updateStrengthIndicator: function () {
        var strength = inputEx.PasswordField.getPasswordStrength(
          this.getValue(),
        );
        for (var i = 0; i < 4; i++) {
          var on = strength >= i * 25 && strength > 0;
          YAHOO.util.Dom.setStyle(
            this.strengthBlocks[i],
            "background-color",
            on ? "#4AE817" : "#FFFFFF",
          );
        }
  ...
  inputEx.PasswordField.getPasswordStrength = function (pw) {
    ...
    var pwstrength =
      pwlength * 10 - 20 + numeric * 10 + numsymbols * 20 + upper * 10;
  ...
  inputEx.messages.invalidPassword = [
    "The password schould contain at least ",
