(function () {
  ...
  inputEx.JsonSchema.Builder.prototype = {
    ...
    schemaToInputEx: function (p, propertyName) {
      var fieldDef = {
        inputParams: { label: propertyName, name: propertyName },
      };
      ...
      if (referencedSchema) {
        ...
        for (var pk in p) {
          if (
            p.hasOwnProperty(pk) &&
            lang.isUndefined(new_schema[pk]) &&
            pk != "$ref"
          ) {
            new_schema[pk] = p[pk];
          }
      ...
      if (p.type) {
        ...
        if (type == "array") {
          ...
          if (lang.isObject(p.items) && !lang.isArray(p.items)) {
            ...
            fieldDef.inputParams.elementType = this.schemaToInputEx(
              p.items,
              propertyName,
            );
          }
          ...
          if (
            !lang.isUndefined(p.pattern) &&
            lang.isUndefined(fieldDef.inputParams.regexp)
          ) {
          ...
          if (
            !lang.isUndefined(p.maxLength) &&
            lang.isUndefined(fieldDef.inputParams.maxLength)
          ) {
            fieldDef.inputParams.maxLength = p.maxLength;
          }

          if (
            !lang.isUndefined(p.minLength) &&
            lang.isUndefined(fieldDef.inputParams.minLength)
          ) {
            fieldDef.inputParams.minLength = p.minLength;
          }

          if (
            !lang.isUndefined(p.readonly) &&
            lang.isUndefined(fieldDef.inputParams.readonly)
          ) {
      ...
      for (var kk in this.defaultOptions) {
        if (
          this.defaultOptions.hasOwnProperty(kk) &&
          lang.isUndefined(fieldDef.inputParams[kk])
        ) {
