...
YAHOO.widget.LogWriter = function (A) {
  if (!A) {
    YAHOO.log(
      "Could not instantiate LogWriter due to invalid source.",
      "error",
      "LogWriter",
    );
    return;
...
YAHOO.widget.LogWriter.prototype.setSource = function (A) {
  if (!A) {
    YAHOO.log(
      "Could not set source due to invalid source.",
      "error",
      this.toString(),
    );
...
YAHOO.widget.LogReader = function (B, A) {
  ...
  if (!this._elContainer) {
    YAHOO.log(
      "Could not instantiate LogReader due to an invalid container element " +
        B,
      "error",
      this.toString(),
    );
  ...
  YAHOO.widget.Logger.categoryCreateEvent.subscribe(
    this._onCategoryCreate,
    this,
  );
...
YAHOO.widget.LogReader.prototype = {
  ...
  formatMsg: function (C) {
    var B = YAHOO.widget.LogReader,
      A =
        this.entryFormat ||
        (this.verboseOutput ? B.VERBOSE_TEMPLATE : B.BASIC_TEMPLATE),
      D = {
        category: C.category,
        label: C.category.substring(0, 4).toUpperCase(),
        sourceAndDetail: C.sourceDetail
          ? C.source + " " + C.sourceDetail
          : C.source,
        message: this.html2Text(C.msg || C.message || ""),
      };
    if (C.time && C.time.getTime) {
      D.localTime = C.time.toLocaleTimeString
  ...
  _initFooterEl: function () {
    ...
    if (this.footerEnabled) {
      ...
      this._elSourceFilters = this._elFt.appendChild(
        document.createElement("div"),
      );
...
if (!YAHOO.widget.Logger) {
  ...
  YAHOO.widget.Logger.logResetEvent = new YAHOO.util.CustomEvent(
    "logReset",
    this,
    true,
  );
