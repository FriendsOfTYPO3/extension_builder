...
class Tx_ExtensionBuilder_Service_CodeGenerator implements t3lib_Singleton {
	 ...
	 * @return Tx_ExtensionBuilder_Service_CodeGenerator
	 */
	public function setSettings($settings) {
		$this->settings = $settings;
		$this->configureRoundtripping();
		return $this;
	}
	 ...
	 * @return void
	 */
	protected function configureRoundtripping() {
	...
	public function build() {
		...
		try {
			$this->moveFile(t3lib_extMgm::extPath('extension_builder') . 'Resources/Private/Icons/ext_icon.gif', $this->extensionDirectory . 'ext_icon.gif');
		} catch (Exception $e) {
		...
		try {
			if (!file_exists($this->extensionDirectory . 'doc/manual.sxw') && file_exists($this->codeTemplateRootPath . 'doc/manual.sxw')) {
				...
				$this->moveFile($this->codeTemplateRootPath . 'doc/manual.sxw', $this->extensionDirectory . 'doc/manual.sxw');
			}
		} catch (Exception $e) {
		...
		try {
			...
			if ($this->extension->hasBackendModules()) {
				foreach ($this->extension->getBackendModules() as $backendModule) {
					/** @var $backendModule Tx_ExtensionBuilder_Domain_Model_BackendModule */
		...
		try {
			...
			$this->moveFile(t3lib_extMgm::extPath('extension_builder') . 'Resources/Private/Icons/relation.gif', $iconsDirectory . 'relation.gif');
		} catch (Exception $e) {
		...
		if (count($this->extension->getDomainObjects()) > 0) {
			...
			try {
				...
				foreach ($this->extension->getDomainObjects() as $domainObject) {
					...
					$this->moveFile(t3lib_extMgm::extPath('extension_builder') . 'Resources/Private/Icons/' . $iconFileName, $iconsDirectory . $domainObject->getDatabaseTableName() . '.gif');
	...
	protected function renderTemplate($filePath, $variables) {
		...
		$renderingContext = $this->buildRenderingContext($variables);
	...
	public function generateDomainObjectCode(Tx_ExtensionBuilder_Domain_Model_DomainObject $domainObject, $mergeWithExistingClass) {
		...
		if ($modelClassObject) {
			...
			return $this->renderTemplate('Partials/Classes/class.phpt', array(
	 ...
	 * @param Tx_ExtensionBuilder_Domain_Model_DomainObject $domainObject
	 * @return string The rendered DocComment
	 */
	 ...
	 * @return string
	 */
	public function generateLocallangModule(Tx_ExtensionBuilder_Domain_Model_BackendModule $backendModule) {
