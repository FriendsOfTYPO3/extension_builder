...
{
    final public const PACKAGE_PREFIX = 'Tx';
    final public const THIS_PACKAGE_KEY = 'GimmeFive';
    final public const DIRECTORY_CLASSES = 'Classes/';
    final public const DIRECTORY_CONFIGURATION = 'Configuration/';
    final public const DIRECTORY_LANGUAGES = 'Resources/Language/';
    final public const DIRECTORY_TEMPLATES = 'Resources/Template/';
    final public const SCOPE_PROTOTYPE = 'prototype';
    final public const SCOPE_SINGLETON = 'singleton';
    ...
    private static ?\Tx_ExtensionBuilder_Tests_Examples_ClassParser_AnotherComplexClass $instance = null;
    ...
    private function loadTypoScriptConfig()
    {
        ...
        $this->tsConf = $TSObj->setup['plugin.']['Tx_Formhandler.']['settings.'];
    }
    ...
    protected function createComponentObject($componentName, array $overridingConstructorArguments)
    {
        ...
        $className = $componentConfiguration['className'] ?: $componentName;
        ...
        $constructorArguments = $componentConfiguration['arguments.'] ?: [];
        ...
        switch ($scope) {
            ...
            case 'prototype' :
                break;
            default :
                throw new Exception('Support for scope "' . $scope . '" has not been implemented (yet)', 1_167_484_148);
        }
        return $componentObject;
    }
    ...
    public function injectConstructorArguments($constructorArguments, &$injectedArguments, &$preparedArguments)
    {
        foreach ($constructorArguments as $index => $constructorArgument) {
            ...
            if (substr((string) $constructorArgument, 0, 3) === self::PACKAGE_PREFIX . '_') {
     ...
     * @param string $componentName
     */
    protected function componentObjectExists($componentName): bool
    {
     ...
     * @param string $componentName
     */
    protected function componentConfigurationExists($componentName): bool
    {
        if (!is_string($componentName)) {
            throw new InvalidArgumentException('The component name must be of type string, ' . gettype($componentName) . ' given.');
        }
    ...
    {
        return is_null($componentConfiguration['scope']) ? 'prototype' : $componentConfiguration['scope'];
    }
        ...
        try {
            ...
            while ($classesDirectoryIterator->valid()) {
                ...
                if ($filename[0] != '.') {
                    ...
                    } elseif (substr($filename, 0, 3) == self::PACKAGE_PREFIX . '_' && str_ends_with($filename, '.php')) {
    ...
    private function loadClass($className)
    {
        ...
        if ($classNameParts[0] === self::PACKAGE_PREFIX) {
            ...
            $classFilePathAndName = $this->classFiles[$className] ?? null;
    ...
    {
        if (!str_contains((string) $packageKey, '/')) {
            $path = ExtensionManagementUtility::extPath(strtolower((string) $packageKey));
        } else {
